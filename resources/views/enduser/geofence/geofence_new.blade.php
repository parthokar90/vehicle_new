...
<head>
    ...
    <link href="{{asset('assets/css/enduser/geofence-custom.css')}}" rel="stylesheet" />
    <!--end::Layout Skins -->
                    ...
                    <div id="kt_aside_menu" class="kt-aside-menu " data-ktmenu-vertical="1" data-ktmenu-scroll="1" data-ktmenu-dropdown-timeout="500">
                        <div class="custom-tabs">
                            <ul class="nav nav-tabs">
                                <li class="nav-item"><a data-toggle="tab" href="#area" class="nav-link active">Area</a></li>
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="area" role="tabpanel">
                                <div class="filtering-area">

                                    <div class="mt-2">
                                        <ul class="list-group">
                                            <li class="list-group-item  d-flex justify-content-between  align-items-center">
                                                <a href="#" class="custom-link-color" id="new-polygon"> <i class="fas fa-plus mr-2"></i> New Area</a>
                                ...
                                <div class="sidebar" style="height: calc(100vh - 105px); overflow:auto;">
                                    ...
                                    <ul class="list-group">
                                        <li class="list-group-item" ng-repeat="p in all_geofences" ng-if="(p.zone_id!=undefined)" ng-click="show_polygon(p.zone_id)" style="cursor: pointer;">@{{p.zone_name}} </li>
                                    </ul>
            ...
            angular.element(document).ready(function() {
                ...
                function load_polygons() {
                    ...
                    $http.get("{{ url('dashboard/get_geofence') }}/" + customer_id).then(function(res) {
                        ...
                        for (var x = 0; x < all_geofence_list.length; x++) {
                            ...
                            var latlng = all_geofence_list[x].zone_vertices.split(",");
                            for(var i=0; i<latlng.length; i++){
                                coords.push(L.latLng(latlng[i],latlng[i+1]));
                                i = i+1;
                            }
                            console.log(coords);
                            polygon.addPolygon(coords, true);
                            //polygon.addPolygon(all_geofence_list[x].coordinates, true);
                            polys[x].zone_id = all_geofence_list[x].zone_id;
                            polys[x].zone_name = all_geofence_list[x].zone_name;
                ...
                $scope.show_polygon = function(id) {
                    for (var x = 0; x < $scope.all_geofences.length; x++) {
                        if ($scope.all_geofences[x].zone_id == id) {
                ...
                function updateCoords(e) {
                    ...
                    for (var j in multi_coords) {
                        var name = $('<h3>' + polys[j].zone_name + '</h3>').addClass(polys[j].zone_name);
                ...
                $('#add_modal').on('show.bs.modal', function() {
                    ...
                    for (var j in multi_coords) {
                        if (pkID!='' && polys[j].zone_id == pkID) {
                            //myCoordinates = multi_coords[j];
                            var coords = multi_coords[j];
                            for (var i in coords) {
