...
<head>
    ...
    <link href="{{asset('assets/css/enduser/profile-custom.css')}}" rel="stylesheet" />
...
<body
    ...
    <!-- begin:: Header -->
    @include('layouts.enduser.profile.navber')
    ...
    //     var page_type = getParameterByName('page_type');
    //     if (page_type != undefined) {
    //         load_profile_page(page_type);
    //         var page_id = '#' + page_type;
    //         $(page_id).parent().addClass('kt-menu__item--active');
    //         $(".profile_menu_item").not(page_id).parent().removeClass('kt-menu__item--active');

    //     } else {
    //         load_profile_page('profile', 'profile_information');
    //         $('#profile').parent().addClass('kt-menu__item--active');
    //     }

    //     $(".profile_menu_item").each(function(index) {
    //         $(this).on('click', function(e) {
    //             e.preventDefault();
	// 			if ($(this).data('rel') == 'profile') {
    //                 load_profile_page($(this).data('rel'), $(this).data('tab'));
    //             } else {
    //                 load_profile_page($(this).data('rel'));
    //             }
    //             $(".profile_menu_item").not($(this)).parent().removeClass(
    ...
    // function load_profile_page(page_name = null , data_tab = null) {
    //     $("#load_content").html(
    //         '<center><div class="my-auto"><img src={{asset("assets/images/loading.gif")}} style=width:130px; height:auto;></div></center>'
    //     ).load("{{ url('dashboard/p')}}/" + page_name + "/" + data_tab);
    // }
