...
function getRouteRaw($imei, $accuracy, $dtf, $dtt)
{
	...
	$q = "SELECT DISTINCT	dt_tracker,	lat,lng,altitude,angle,speed,params	FROM `gs_object_data_" . $imei . "` WHERE dt_tracker BETWEEN '" . $dtf . "' AND '" . $dtt . "' group by dt_tracker, lat, lng ORDER BY dt_tracker ASC";
	...
	foreach ($r as $route_data) {
		...
		// unit_distance will be set as globally. in GPS server it was set as:  $_SESSION["unit_distance"]. i.e: meter, mile km etc
		//$unit_distance = 'mi';
		$speed = convSpeedUnits($speed, 'km', $_SESSION["unit_distance"]);
		//$speed = convSpeedUnits($speed, 'km', $unit_distance);
	...
	// fuel smoothening	
	if (is_array($fuel_sensors)) {
		for ($i = 0; $i < count($fuel_sensors); ++$i) {
			$param = $fuel_sensors[$i]['param']; //i.e: accv

			for ($j = 0; $j < count($route); ++$j) // $route stores all data from gs_object_data_<imei> table
			{
				...
				//i.e: prev accv>current accv and current_accv<next_accv
				{
					if (isset($route[$j])) {
...
{
	// in gps server sensor identified by $imei 
	//but in our platform $imei = model_id 

	$result = array();

	$q = "SELECT s.*, p.name as sensor_parameter_name FROM gps_sensors s left join master_settings p on p.id = s.sensor_parameter_id where s.status=1 and s.model_id=(select model_id from gs_objects where imei='$imei') and p.name = '$type'";
	$r = \DB::select($q);
...
function getObjectSensors($imei)
{
	...
	$q = "SELECT s.*, m.name as model_name, sv.name as server_name, sn.name as sensor_name, st.name as sensor_type_name, sp.name sensor_parameter_name, sd.name as sensor_default_name, r.name as result_type_name FROM gps_sensors s left join device_models m on m.id=s.model_id left join gps_servers sv on sv.id = s.server_id left join master_settings sn on sn.id = s.sensor_name_id left join master_settings st on st.id = s.sensor_type_id left join master_settings sp on sp.id = s.sensor_parameter_id left join master_settings sd on sd.id = s.sensor_default_id left join master_settings r on r.id = s.result_type_id where model_id=(select device_model from gs_objects where imei='$imei')";
	$r = \DB::select($q);
	...
	foreach ($r as $row) {
		...
		$sensors[$sensor_id] = array(
			...
			'type' => $row->sensor_type_name,
			'param' => $row->sensor_parameter_name,
			'data_list' => ($row->is_data_list == 1) ? true : false,
			'popup' => ($row->is_popup == 1) ? true : false,
			'result_type' => $row->result_type_name,
			'text_1' => $row->sensor_1_text,
			'text_0' => $row->sensor_0_text,
			'units' => $row->unit_of_measurement,
			'lv' => $row->lowest_value,
			'hv' => $row->highest_value,
			'acc_ignore' => ($row->ignore_ignition == 1) ? true : false,
			'formula' => $row->formula,
...
function getRoute($imei, $dtf, $dtt, $min_stop_duration, $filter)
{
	...
	if (count($route) > 0) {
		...
		$result['route'] = $route;
		//echo var_export($result['route'],true);
		// get fuel sensors
		$fuel_sensors = getSensorFromType($imei, 'fuel');
		$fuelcons_sensors = getSensorFromType($imei, 'fuelcons');
		...
		for ($i = 0; $i < count($result['drives']); ++$i) {
			$result['route_length'] += round($result['drives'][$i][7]); // make route_length round figure
		}
		//echo $result['route_length'].PHP_EOL;
		// count top speed				
		$result['top_speed'] = 0;
		for ($i = 0; $i < count($result['drives']); ++$i) {
...
function getRouteDrives($route, $accuracy, $stops, $fcr, $fuel_sensors, $fuelcons_sensors, $acc)
{
	...
	if (count($stops) == 0) {
		...
		$dt_start_s = $route[$id_start_s][0]; // dt_tracker(i.e: 2020-08-22 00:00:00)
		$dt_start = $route[$id_start][0]; // dt_tracker(i.e: 2020-08-22 00:00:00)
		$dt_end = $route[$id_end][0]; // dt_tracker(i.e: 2020-08-23 23:59:00)
...
function getRouteFuelConsumption($route, $id_start, $id_end, $fcr, $fuel_sensors, $fuelcons_sensors)
{
	...
	//return (100-(round($fuel_consumtion * 100) / 100));
}
